...
describe('selectionPosition', function() {
	...
	describe('image', function() {
		it('toolbar should show centered when the selection is in an image', function() {
			...
			var config = getSelectionsConfig(
				AlloyEditor.SelectionSetPosition.image
			);

			var nativeEditor = this.editor.get('nativeEditor');

			var editorEvent = getPayload.call(
				this,
			...
			assert.ok(
				endPoint[0] > (TEST_OBJECT_WIDTH - TEST_TOOLBAR_WIDTH) / 2
			);
		});
	});
	...
	describe('table', function() {
		it('toolbar should show centered when the selection is in a table', function() {
			bender.tools.selection.setWithHtml(
				this.nativeEditor,
				'<table id="testTable" width="' +
					TEST_OBJECT_WIDTH +
					'" style="position: absolute; left: 0; top: 0;"><tbody><tr><td> {}</td></tr></tbody></table>'
			);

			var config = getSelectionsConfig(
				AlloyEditor.SelectionSetPosition.table
			);
