...
import {ClayAlert, ClayStripe, ClayToast} from 'clay-alert';
import {
	ClayDropdown,
	ClayActionsDropdown,
...
class CodeEditor extends Component {
	...
	render() {
		const {code, showBabelErrorMessage, error} = this.state;
		...
		if (showBabelErrorMessage) {
			errorMessage = (
				<span>
					...
					<br />
					<br />
					This can be caused by an ad blocker. If you are using one,
					consider adding <b>clayui.com</b> to the whitelisted so that
					try can work again.
				</span>
			);
		} else if (error != null) {
		...
		return (
			<LiveProvider code={code} mountStylesheet={false}>
				<div className="try-playground-editor">
					<span className="try-playground-title">Live Editor</span>
				...
				{error && (
					<div className="try-playground-live try-live-error">
						<span className="try-playground-title">Error</span>
				...
				{!error && (
					<div className="try-playground-live">
						<span className="try-playground-title">Result</span>
						<div
							className="try-playground-live--preview"
							ref="mountRef"
						/>
					</div>
				)}
