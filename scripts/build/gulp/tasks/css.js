...
gulp.task('sass2css', function() {
    return gulp.src(path.join(Constants.rootDir, 'src/assets/sass/**/main.scss'))
        .pipe(sass({
            includePaths: [
                path.join(Constants.rootDir, 'node_modules/bourbon/app/assets/stylesheets'),
...
gulp.task('join-css', function() {
    ...
    var skins = getFolders(path.join(Constants.editorDistFolder, 'assets/css/skin'));
    ...
    var tasks = skins.map(function(skin) {
        ...
        return gulp.src(
            [
                path.join(Constants.rootDir, 'src/assets/sass/skin', skin, '.font-cache', skinFontFileName),
...
gulp.task('minimize-css', function() {
    return gulp.src(path.join(Constants.editorDistFolder, 'assets/*.css'))
        .pipe(minifyCSS())
        .pipe(rename({
            suffix: '-min'
        }))
...
gulp.task('clean-fonts', function(callback) {
    del([
        path.join(Constants.editorDistFolder, 'assets/css')
