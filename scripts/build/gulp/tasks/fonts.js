...
gulp.task('generate-icon-fonts', function() {
	var tasks = skins.map(function(skin) {
		...
		if (shouldUpdateSkinIconFont(skin)) {
			return gulp
				.src(path.join(skinFontDir, 'tmp', 'icons', '*.svg'))
				...
				.on('glyphs', function(glyphs, options) {
					gulp.src(
						path.join(
							Constants.assetsDir,
							'font/font-icon-map-template.scss'
						)
					)
						.pipe(
							consolidate('lodash', {
					...
					gulp.src(
						path.join(Constants.assetsDir, 'font/font-template.css')
					)
						.pipe(
							consolidate('lodash', {
								fontName: fontName,
				...
				.pipe(
					gulp.dest(
						path.join(Constants.editorDistFolder, 'assets', 'fonts')
					)
				);
		} else {
			// No need to regenerate the icon font, just copy to dist folder
			return gulp
				.src(path.join(skinFontDir, 'fonts', '*.*'))
				.pipe(
					gulp.dest(
						path.join(Constants.editorDistFolder, 'assets', 'fonts')
					)
