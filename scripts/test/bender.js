...
(function(bender) {
	...
	function browserHtmlFix(html) {
		if (
			...
			html = html.replace(/href="(.*?)#(.*?)"/gi, function(
				m,
				base,
				anchor
			) {
	...
	bender.tools = {
		...
		getInnerHtml: function(elementOrId, stripLineBreaks) {
			...
			if (typeof elementOrId == 'string') {
				...
				html = elementOrId.innerHTML || elementOrId.value; // retrieve from innerHTML // retrieve from value
			}
		...
		fixHtml: function(html, stripLineBreaks, toLowerCase) {
			...
			html = html.replace(/(<\b(?!!)[^>\s]*)(.*?)(\s*\/?>)/g, function(
				match,
				start,
				body,
				end
		...
		compatHtml: function(
			...
			if (fixStyles) {
				html = html.replace(/ style="([^"]+)"/g, function(
					match,
					style
				) {
		...
		setUpEditors: function(editorsDefinitions, callback) {
			...
			function next() {
				...
				bender.editorBot.create(editorsDefinitions[name], function(
					bot
				) {
					bots[name] = bot;
		...
		createTestsForEditors: function(editors, inputTests) {
			...
			for (var editorName in editors) {
				for (var testName in inputTests) {
					...
					specificTestName =
						testName + ' (' + editors[editorName].name + ')';
	...
	bender.tools.range = {
		...
		getWithHtml: (function() {
			...
			function injectComment(marker, node, offset) {
				var comment = new CKEDITOR.dom.comment(
					'cke-range-marker-' + marker
				);
...
bender.tools.getData = function(editor, config) {
